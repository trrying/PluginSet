...
class SvnIgnorePlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        project.rootDir.list().each { if (project.file("${project.rootDir}/$it/build").exists()) { ignoredMap.add(["name": "path", "value": "$it/build/"]) } }
        ...
        def changeListManager = workspace.component.find { it.attribute("name") == "ChangeListManager" } ?: workspace.appendNode("component", ["name": "ChangeListManager"])
        ignoredMap.each {
